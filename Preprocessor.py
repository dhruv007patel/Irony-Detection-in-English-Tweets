...
class Preprocess(object):
    ...
    def process_data(self, train_file, test_file, load_saved_data=True):
        ...
        pos_tags = []
        n_train = 0
        train_reader = open(train_file, 'r', encoding="ISO-8859-1")
        for line in train_reader:
            elements = line.split("\t")
            elements[2] = self.normalise_tweet(elements[2])
            processed_data_file.write(str(elements[2].encode("utf8")) + "\n")
        ...
        test_reader = open(test_file, 'r', encoding="ISO-8859-1")
        for line in test_reader:
            elements = line.split("\t")
            elements[1] = self.normalise_tweet(elements[1])
            processed_data_file.write(str(elements[1].encode("utf8")) + "\n")
    ...
    def load_normalisation_dict(self):
        reader = open(self.ROOT_DIR + "data/normalisation/emnlp_dict.txt", encoding="ISO-8859-1")
        for line in reader:
            elements = re.split("\\s+", line)
            self.normalisation_dict[elements[0].strip()] = elements[1].strip()
        reader.close()
        reader = open(self.ROOT_DIR + "data/normalisation/Test_Set_3802_Pairs.txt", encoding="ISO-8859-1")
        for line in reader:
    ...
    def load_sentiment_words(self):
        reader = open(self.ROOT_DIR + "data/sentiment/negative-words.txt", encoding="ISO-8859-1")
        for line in reader:
            if len(line.strip()) > 0:
                self.negative_set.add(line.strip())
        reader.close()

        reader = open(self.ROOT_DIR + "data/sentiment/positive-words.txt", encoding="ISO-8859-1")
        for line in reader:
            if len(line.strip()) > 0:
    ...
    def load_dict(filename):
        with open(filename, 'rb', encoding="ISO-8859-1") as f:
    ...
    def process_a_tweet(self, tweet_str):
        ...
        if type(tweet_str) != str:
            tweet_str = tweet_str.encode('utf8')

        tweet_str = tweet_str
        n_token = len(re.split("\\s+", tweet_str.lower()))
        embedding_vector = self.embedding_model(tweet_str)
    ...
    def add_tweet_text(predicted_file, input_file):
        tweet_file = open(input_file, "r", encoding="ISO-8859-1")
        tweet_map = {}
        ...
        label_file = open(predicted_file, "r", encoding="ISO-8859-1")
